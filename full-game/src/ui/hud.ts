export interface HudData {
  ...
  coins: number;
  waveNumber: number;
  // The number of designed (non-synthesized) waves — 25 in the full game.
  // Bug fix: this used to be a hardcoded "/5" left over from the prototype
  // copy, which made the real 25-wave table look capped at 5. Now sourced
  // from WAVES.length so it stays correct if the table's length ever changes.
  totalWaves: number;
  // True once past the last static WAVES entry (endless mode) — HUD drops
  // the "/N" suffix and shows an "Endless" tag instead once true.
  isEndless: boolean;
  enemiesAlive: number;
...
export function drawHud(ctx: CanvasRenderingContext2D, screenW: number, screenH: number, d: HudData): void {
  ...
  ctx.fillStyle = '#fff';
  ctx.fillText(d.isEndless ? `Wave ${d.waveNumber} (Endless)` : `Wave ${d.waveNumber}/${d.totalWaves}`, 16, 76);
  ctx.font = '26px sans-serif';
  ctx.fillStyle = '#cfd8e3';
